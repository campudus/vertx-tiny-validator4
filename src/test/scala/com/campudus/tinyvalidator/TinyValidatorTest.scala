 ...
 */
package com.campudus.tinyvalidator
...
class TinyValidatorTest extends TestVerticle {
  ...
  @Ignore @Test def addSchemaWithInvalidJson(): Unit = {
  ...
  @Test def invalidJson(): Unit = {
    vertx.eventBus.send(address, Json.obj("action" -> "validate", "key" -> "schema0", "json" -> invalidSimpleJson), {
      msg: Message[JsonObject] =>
        ...
        //println(msg.body.getValue[Map[String, String]]("report"))
        testComplete()
    })
  }
  ...
  @Ignore @Test def noRealJsonParameter: Unit = {
    vertx.eventBus.send(address, Json.obj("action" -> "validate", "key" -> "testschema", "json" -> "asdf"), {
      msg: Message[JsonObject] =>
        assertEquals("error", msg.body.getString("status"))
